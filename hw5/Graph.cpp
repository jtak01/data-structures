...
// Returns: nothing
// Does: constructor. Initializes with num_v vertices
template<class Vertex>
...
Graph<Vertex> &Graph<Vertex>::operator= (const Graph &source)
{
    if (this != &source)
    {
        for (int i = 0; i < num_vertices; i++)
        {
            if (source.path != NULL and this -> path != NULL)
                this -> path[i] = source.path[i];
        }
    }
    return *this;
}
...
template<class Vertex>
Vertex Graph<Vertex>::get_vertex(int i) const
{
    return vertices[i];
}
...
//       (in case the edge did not exist) 
 template<class Vertex>
 void Graph<Vertex>::add_edge(Vertex f, Vertex t, string name)
 {
     int row;
     int col;
...
{
    int row = index_is(vertices, f);
    int col = index_is(vertices, t);
