...
bool Maze::solve(Position p)
{
        ...
        if (mazeAt(p) == FINISH)
                return true;
        if (!p.inBounds(maze_array_size, maze_array_size))
                return false;
        if (mazeAt(p) == REJECT or mazeAt(p) == WALL or mazeAt(p) == PATH)
                return false;
        else {
...
{
        if (row > maze_array_size or col > maze_array_size)
                return 0;
        else
        {
                if (maze[row][col] == PATH)
                        return 1 + num_dots_in_row(row, col + 1);
                else return num_dots_in_row(row, col + 1);
        }
}
